...
export const StatusChangeModal: React.FC<StatusChangeModalProps> = ({
  ...
  const handleApply = (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((tender.status === 'AWARDED' || tender.status === 'REJECTED') && currentUser.role !== 'SUPER_ADMIN') {
        throw new Error('Only Super Admin can revert an Awarded or Rejected tender.');
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in-50 duration-100">
      <div className="bg-white rounded-xl shadow-2xl border border-slate-200 max-w-lg w-full overflow-hidden">
        ...
        <form onSubmit={handleApply} className="p-5 space-y-4">
          {error && (
            <div className="p-3 bg-rose-50 border border-rose-200/80 rounded-lg text-xs text-rose-800 flex items-start gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {allowed.map((st) => (
                  <button
                    ...
                    className={`py-2 px-3 rounded-lg text-xs font-medium border transition-colors text-left cursor-pointer ${
                      targetStatus === st
                        ? 'border-[#8b151b] bg-[#8b151b] text-white shadow-xs'
          ...
          {targetStatus === 'REJECTED' && (
            <div className="p-3.5 bg-rose-50/50 border border-rose-200/80 rounded-xl space-y-3">
              ...
              <div>
                ...
                <select
                  ...
                  required
                  className="w-full text-xs p-2 rounded-lg border border-slate-200 bg-white text-slate-900 outline-none"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-xs p-2 rounded-lg border border-slate-200 bg-white font-mono text-slate-900 outline-none"
                  />
                </div>
          ...
          {tender.status === 'AWARDED' && targetStatus !== 'AWARDED' && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl space-y-2">
              ...
              <textarea
                ...
                className="w-full text-xs p-2 rounded-lg border border-amber-300 bg-white text-slate-900 outline-none"
              />
            </div>
          )}
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
            ...
            <button
              ...
              className="px-4 py-1.5 rounded-lg text-xs font-semibold text-white bg-[#8b151b] hover:bg-[#731217] transition-colors shadow-xs cursor-pointer disabled:opacity-40"
            >
